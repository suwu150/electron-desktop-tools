...
const data = translate(menuData, 'sortCode', 'parent', 'children', 'level');

/*
 * 树结构广度优先遍历
 *   data: 树结构数据
 * */
function breadthFirstTraversal (data) {
    let result = [];
    const menuData = data.slice(0);

    const sortMenuData = menuData
        .filter(item => item.sortCode.trim().split('-').length === 1)
        .sort((a, b) => a-b);
    result = result.concat(sortMenuData);
...
console.log(breadthFirstTraversal(data));
